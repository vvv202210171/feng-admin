...
import CreateForm from './modules/FlashCoinForm'
...
const modalOptions = [
  ...
  },
  {
    ...
    field: 'height',
    type: 'number',
    placeholder: '请输入单日最高限额',
    decorator: { rules: [{ required: true, message: '请输入单日最高限额' }] }
  },
  {
    title: '单次最低限额',
    field: 'low',
  ...
  },
  {
    ...
    type: 'number',
    placeholder: '请输入主币单价',
    decorator: { rules: [{ required: true, message: '请输入主币单价' }] }
  },
  {
    title: '交易币单价',
    field: 'tradprice',
    type: 'number',
    placeholder: '请输入交易币单价',
    decorator: { rules: [{ required: true, message: '请输入交易币单价' }] }
  }
]
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await getFlashCoinList(this.queryParam)
      const { code, data } = res
      if (code === 0) {
