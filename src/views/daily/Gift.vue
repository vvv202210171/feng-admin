<template>
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      >
        ...
        <a-form-item label="类型">
          <a-select v-model="queryParam.type" style="width: 120px" allow-clear>
            <a-select-option
              :value="item.code"
              v-for="item in enums['MemberGiftTypeEnum']"
            >
              {{ item.desc }}
            </a-select-option>
          </a-select>
      ...
      >
        <p slot="gift_time" slot-scope="record" style="margin: 0">
          {{ record | dayjs }}
        </p>
        <p slot="type" slot-scope="record" style="margin: 0">
          <a-tag :color="record == 'self' ? 'blue' : 'green'">
            {{ getDesc("MemberGiftTypeEnum", record) }}
          </a-tag>
        </p>
      </a-table>
    </a-card>
...
const columns = [
  ...
  },
  {
    dataIndex: "gift_time",
    title: "时间",
    scopedSlots: { customRender: "gift_time" },
    width: 150,
  },
];
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await getGiftRecord(this.queryParam);
      const { code, data } = res;
      if (code === 0) {
