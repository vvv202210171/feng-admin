    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <a-form-item label="类型">
          <a-select v-model="queryParam.status" style="width: 120px" allow-clear>
      ...
      >
        ...
        <p slot="type" slot-scope="type" style="margin: 0">
          {{ type == "buy" ? "买单" : "卖单" }}
        </p>
        <template slot="maincoin" slot-scope="text, record">
          {{ record.tradcoin }}/{{ record.maincoin }}
        </template>
        <p slot="status" slot-scope="record" style="margin: 0">
          <a-tag :color="record == 1 ? 'blue' : 'green'">
            {{ record == 1 ? "结算" : "未结算" }}
          </a-tag>
        </p>
      </a-table>
...
const columns = [
  {
    dataIndex: "maincoin",
    title: "交易对",
    scopedSlots: { customRender: "maincoin" },
    width: 150,
  },
  {
    dataIndex: "member",
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await getDailyFee(this.queryParam);
      const { code, data } = res;
      if (code === 0) {
