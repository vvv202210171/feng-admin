    ...
    <a-form id="formLogin" class="user-layout-login" ref="formLogin" :form="form" @submit="handleSubmit">
      ...
      >
        <a-tab-pane key="tab1" :tab="$t('user.login.tab-login-credentials')">
          ...
          <a-form-item v-if="enableGoogleCode">
            <a-input
              size="large"
              autocomplete="off"
              type="text"
              ...
              v-decorator="[
                'code',
                { rules: [{ required: true, message: $t('user.verification-code.required') }], validateTrigger: 'blur' }
              ]"
            >
...
import { getip, getSmsCaptcha } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    requestFailed(err) {
      ...
      const desc = err.msg || '请求出现错误，请检查网络'
      this.loginErrorMsg = desc
      // this.$notification['error']({
      //   message: '错误',
      //   description: ((err.response || {}).data || {}).msg || '请求出现错误，请检查是否输入错误',
      //   duration: 4
      // })
    }
  }
