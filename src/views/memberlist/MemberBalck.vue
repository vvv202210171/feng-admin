  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      <a-form name="configSearch" layout="inline" autocomplete="off" style="margin-bottom: 14px" @submit="doSearch">
        ...
        <a-form-item>
          <a-button type="primary" icon="plus" @click="handleAdd">新建黑名单</a-button>
          <a-divider type="verticle" />
          <a-button type="danger" icon="plus" @click="handleBatchDel">批量删除</a-button>
        </a-form-item>
      </a-form>
...
import CreateForm from './modules/BlackIpForm'
  ...
  methods: {
    ...
    handleDel(record) {
      ...
      const delMod = Modal.confirm({
        ...
        maskClosable: true,
        title: '提示信息',
        content: `确定删除【${record.ip}】的记录？`,
        onOk: () => {
          delBlackIp({
            autoids: record.autoid
          }).then((res) => {
            console.log(res)
            ts.getData()
            delMod.destroy()
    ...
    handleBatchDel() {
      const ts = this
      if (!ts.selectedRows || ts.selectedRows.length < 0) {
        return
      }
      const autoids = ts.selectedRows.map((v) => v.autoid).join()
      const delMod = Modal.confirm({
        centered: true,
        maskClosable: true,
        title: '提示信息',
        content: `确定删除【${autoids}】的记录？`,
        onOk: () => {
          delBlackIp({
            autoids: autoids
          }).then((res) => {
    ...
    async getData() {
      this.loading = true
      const res = await getBlackIp(this.queryParam)
      const { code, data } = res
      if (code === 0) {
