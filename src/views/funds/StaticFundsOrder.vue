...
import { getOrderStaticList, orderStatus } from '@/api/funds'
...
export default {
  ...
  async created() {
    const s = await orderStatus()
    this.orderStatus = s.data
    this.getData()
  },
  computed: {
    // 搜索状态
  ...
  methods: {
    getStateDesc(record) {
      const obj = this.orderStatus.find((v) => v.code == record.state)
      return obj ? obj.desc : record.state
    },
    copy(iconName) {
      navigator.clipboard
        .writeText(iconName)
    ...
    async getData() {
      ...
      const res = await getOrderStaticList(this.queryParam)
      const { code, data } = res
      if (code === 0) {
