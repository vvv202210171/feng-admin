...
import { getHelpStaticList, staticHelpExam } from '@/api/funds'
...
export default {
  ...
  methods: {
    ...
    async handleEdit(state, record) {
      const { code, msg } = await staticHelpExam({ state, id: record.id })
      if (code === 0) {
    ...
    async getData() {
      ...
      const res = await getHelpStaticList(this.queryParam)
      const { code, data } = res
      if (code === 0) {
