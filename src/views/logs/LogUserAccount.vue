    ...
    <a-card :bordered="false">
      ...
      <a-form name="Search" layout="inline" autocomplete="off" style="margin-bottom: 14px" @submit="doSearch">
        ...
        <a-form-item label="币种">
          <a-input v-model="queryParam.coin" style="width: 100px" placeholder="请输入要搜索的币种" />
        </a-form-item>
        ...
        <a-form-item label="来源">
          <a-select v-model="queryParam.sourceType" style="width: 180px" allow-clear>
            <a-select-option :value="item.code" v-for="item in enums['SourceTypeEnum']" :key="item.code">
              {{ item.desc }}
            </a-select-option>
          </a-select>
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await getLogUserAccountList(this.queryParam)
      const { code, data } = res
      if (code === 0) {
